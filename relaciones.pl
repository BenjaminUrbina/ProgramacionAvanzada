es_documento_de('paradigmasProgramacion', 'Gonzalo Honores').
es_documento_de('paralelismo', 'Gonzalo Honores').
es_documento_de('proyectos', 'Gonzalo Honores').
es_documento_de('prueba chi cuadrado', 'Carlos Diaz').
es_documento_de('Prueba01', 'Benjamin Urbina').
es_documento_de('taller 3', 'Carlos Diaz').
es_documento_de('Prueba011', 'Benjamin Urbina').
es_documento_de('cobit19', 'roberto godoy').
es_documento_de('Prueba01133', 'Benjamin Urbina').
